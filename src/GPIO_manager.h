...
namespace GPIO_manager
{
    ...
    gpio_pin available_gpio_list[] {
        ...
        {3,PinMode::NA},  // RXD0 (reserved for programming/UART) only safe to use as a input
...
#endif
    uint8_t available_gpio_list_lenght = sizeof(available_gpio_list)/sizeof(available_gpio_list[0]);
    uint8_t PinModeStrings_lenght = sizeof(PinModeStrings)/sizeof(PinModeStrings[0]);
...
#endif
        server = &srv;
        srv.on("/GPIO_manager/getAvailableGPIOs", HTTP_GET, sendList);
    }
}